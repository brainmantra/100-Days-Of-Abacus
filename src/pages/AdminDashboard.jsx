  ...
  const handleRenameSection = async () => {
    ...
    if (match) {
      if (confirm(`Are you sure you want to rename "${oldSectionName}" to "${newSectionName}"? This will update the saved section name in the database.`)) {
        try {
          await adminApi.delete(`/admin/teacher-questions?level=${qLevel}&day_number=${qDay}&section=${encodeURIComponent(oldSectionName)}`)
          ...
          <label className="form-label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {qSection && !getTeacherSectionsForLevel(qLevel, qDay).some(s => s.value === qSection) && (
              <button 
                ...
                style={{ padding: '0 4px', fontSize: '0.75rem', height: 'auto', textDecoration: 'underline', color: 'var(--primary)', minHeight: 'unset' }}
              >
                ✏ Rename Section
              </button>
            )}
